...
export default function SidebarContent({ enrollments }) {
    return (
        <div className="flex-1 overflow-y-auto p-8 pl-14 bg-white">
            <div className="flex flex-col gap-6">
                {enrollments.map((item) => (
                    <div key={item.id} className="card p-6 rounded-3xl shadow-sm bg-white flex flex-col gap-4">
                        <div className="flex gap-6">
                            <div className="w-[269px] h-[200px] ">
                                <img
                            ...
                            <div className="flex-grow pt-2">
                                <div className="flex justify-between items-start mb-1">
                                    <p className="text-sm text-slate-500">
                                        Lecturer <span className="text-slate-800 font-semibold">{item.course.instructor.name}</span>
                                    </p>
                                    <div className="flex items-center gap-1 text-amber-500 font-bold">
                                        <Star className="w-5 h-5 fill-current" />
                                        <span className="text-slate-700">{item.course.avgRating.toFixed(1)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex-col gap-2 mt-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-gray-500">{item.schedule?.location || "Google Meet"}</span>
                                    </div>
                                    <div className="ml-1.5 flex items-center gap-2">
                                        <span className="text-black text-xl">$</span>
                                        <span className="text-gray-500 ml-1.5">{Number(item.totalPrice).toFixed(0)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="flex items-end gap-8">
                            ...
                            <Link to={`/course/${item.course.id}`} className="flex-shrink-0 px-10 py-2.5 rounded-xl border-2 border-[#a8a2f8] text-[#5d51e8] font-bold text-lg hover:bg-indigo-50 transition-colors">
                                View
                            </Link>
                        </div>
                    </div>
